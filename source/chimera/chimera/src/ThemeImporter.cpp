...
void ThemeImporter::Import(const string& path, const string& parent, vector<SassImportEntry>& files) {
	if (!StringUtil::StartsWith(path, "theme:")) return;
	...
	if (StringUtil::StartsWith(file, "..")) throw SassError(fmt::format("File to import not allowed: {}", file));

	// Find the file(s).
	vector<string> found = PathUtil::Expand(path, this->theme.directory());
